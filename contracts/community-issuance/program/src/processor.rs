...
impl Processor {  
    ...
    pub fn process_initialize(
        ...
        if *authority_info.key != Self::authority_id(program_id, community_issuance_info.key, nonce)? {
            return Err(CommunityIssuanceError::InvalidProgramAddress.into());
        }
        ...
        if *community_issuance_info.owner != *program_id {
            return Err(CommunityIssuanceError::InvalidOwner.into());
        }
    ...
    pub fn authority_id(
        ...
        my_info: &Pubkey,
        nonce: u8,
    ) -> Result<Pubkey, CommunityIssuanceError> {
        Pubkey::create_program_address(&[&my_info.to_bytes()[..32], &[nonce]], program_id)
            .or(Err(CommunityIssuanceError::InvalidProgramAddress))
    }
...
impl PrintProgramError for CommunityIssuanceError {
    fn print<E>(&self)
    where
        E: 'static + std::error::Error + DecodeError<E> + PrintProgramError + FromPrimitive,
    {
        match self {
            CommunityIssuanceError::AlreadyInUse => msg!("Error: The account cannot be initialized because it is already being used"),
            CommunityIssuanceError::InvalidProgramAddress => msg!("Error: The program address provided doesn't match the value generated by the program"),
            CommunityIssuanceError::InvalidState => msg!("Error: The stake pool state is invalid"),
            CommunityIssuanceError::InvalidOwner => msg!("Error: Pool token account's owner is invalid"),
            CommunityIssuanceError::InvalidPoolToken => msg!("Error: Given pool token account isn't same with pool token account"),
        }
    }
} 

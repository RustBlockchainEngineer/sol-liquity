...
impl Decimal {
    ...
    pub fn to_scaled_val(&self) -> Result<u128, ProgramError> {
        Ok(u128::try_from(self.0).map_err(|_| StabilityPoolError::MathOverflow)?)
    }
    ...
    pub fn try_round_u64(&self) -> Result<u64, ProgramError> {
        let rounded_val = Self::half_wad()
            ...
            .ok_or(StabilityPoolError::MathOverflow)?;
        Ok(u64::try_from(rounded_val).map_err(|_| StabilityPoolError::MathOverflow)?)
    }
    ...
    pub fn try_ceil_u64(&self) -> Result<u64, ProgramError> {
        let ceil_val = Self::wad()
            ...
            .checked_div(Self::wad())
            .ok_or(StabilityPoolError::MathOverflow)?;
        Ok(u64::try_from(ceil_val).map_err(|_| StabilityPoolError::MathOverflow)?)
    }
